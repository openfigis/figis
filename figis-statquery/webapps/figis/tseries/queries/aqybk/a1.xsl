...
<xsl:variable name="TableID" select="'A-1'"/>
<xsl:variable name="TableCode" select="'A-1'"/>
...
<xsl:template match="Body/RowSet/RowSet">
	...
	<xsl:apply-templates select="Row[Key/@gid='1:21001:200' and @ser='Quantity']" mode="tworows">
		<xsl:with-param name="elementname" select="'figis:tablerow'"/>
		<xsl:with-param name="keyNo" select="4"/>
	</xsl:apply-templates>
	<xsl:apply-templates select="Row[Key/@gid='1:21001:201' and @ser='Quantity']" mode="tworows">
		<xsl:with-param name="elementname" select="'figis:tablerow'"/>
		<xsl:with-param name="keyNo" select="4"/>
	</xsl:apply-templates>
</xsl:template>
